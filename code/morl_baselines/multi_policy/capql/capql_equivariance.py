...
from morl_baselines.common.networks import layer_init, mlp, polyak_update, mlp_no_bias
...
SYMMETRY_BLUEPRINTS = {
    'mo-walker2d-v5': {
        'permutation': {
            ...
            'limb_obs_dim': 6,
            'limb_action_dim': 3,
            'root_obs_indices': [0, 1, 8, 9, 10],
            'limbs_obs_indices': [
                [2, 3, 4, 11, 12, 13],  # Right Leg
                [5, 6, 7, 14, 15, 16],  # Left Leg
            ],
            'root_action_dim': 0,
            'limbs_action_indices': [
                [0, 1, 2],  # Right Leg
                [3, 4, 5],  # Left Leg
            ]
        },
        'reflectional': {
            'sym_obs_indices': [2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16],
            'asym_obs_indices': [0, 1, 8, 9, 10],
            'sym_action_indices': [0, 1, 2, 3, 4, 5],
            'asym_action_indices': [],
        }
    },
    'mo-hopper-v5': {
        'permutation': None,
        'reflectional': {
            'sym_obs_indices': [2, 3, 4,8, 9, 10],
            'asym_obs_indices': [0, 1, 5,6, 7],
            'sym_action_indices': [0, 1, 2],
            'asym_action_indices': [],
        }
    },
    'mo-halfcheetah-v5': {
    ...
    'mo-swimmer-v5': {
        'permutation': None,
        'reflectional': {
            'sym_obs_indices': [1, 2, 4, 6, 7],
            'asym_obs_indices': [0, 3, 5],
            'sym_action_indices': [0, 1],
            'asym_action_indices': [],
        }
    },
}
...
class ReflectionalSymmetricPolicy(nn.Module):
    """
    ...
    def __init__(self, obs_dim, rew_dim, output_dim, action_space, net_arch,env_name, use_enc, **kwargs):
        ...
        # --- Store the Detailed Index Blueprint ---
        sym_obs_indices = SYMMETRY_BLUEPRINTS[env_name]['reflectional']['sym_obs_indices']
        asym_obs_indices = SYMMETRY_BLUEPRINTS[env_name]['reflectional']['asym_obs_indices']
        sym_action_indices = SYMMETRY_BLUEPRINTS[env_name]['reflectional']['sym_action_indices']

    
        self.register_buffer('sym_obs_indices', th.tensor(sym_obs_indices, dtype=th.long))
        self.register_buffer('asym_obs_indices', th.tensor(asym_obs_indices, dtype=th.long))
        

        self.register_buffer('sym_action_indices', th.tensor(sym_action_indices,dtype=th.long))
        self.register_buffer('asym_action_indices', th.tensor([],dtype=th.long))
        ...
        # Final output heads for the action parameters
        self.mean_head = nn.Linear(net_arch[-1], output_dim)
        self.log_std_head = nn.Linear(net_arch[-1], output_dim)

        self.apply(layer_init)

    def reflect_action(self, action):
        """Apply K_g transformation: negate symmetric actions"""
        reflected_action = action.clone()
        if len(self.sym_action_indices) > 0:
            reflected_action[..., self.sym_action_indices] *= -1
        return reflected_action

    def reflect_state(self, obs):
        """Apply L_g transformation: negate symmetric states"""
        reflected_obs = obs.clone()
        reflected_obs[..., self.sym_obs_indices] *= -1
        return reflected_obs

    def compute_equivariance_loss(self, obs, w):
        """Enforce π(L_g(s)) = K_g(π(s))"""
        # Original policy output
        mean_original, log_std_original = self.forward(obs, w)
        
        # Reflected state input
        obs_reflected = self.reflect_state(obs)
        mean_reflected, log_std_reflected = self.forward(obs_reflected, w)
        
        # What the reflected action should be
        expected_reflected_mean = self.reflect_action(mean_original)
    ...
    def forward(self, obs, w):
        ...
        is_batched = obs.dim() > 1
        if not is_batched:
            obs = obs.unsqueeze(0)
            w = w.unsqueeze(0)

        # Partition observation into symmetric and asymmetric parts 
        s_sym = obs[:, self.sym_obs_indices]
        s_asym = obs[:, self.asym_obs_indices]
        ...
        # Process combined info through the main network
        latent_pi = self.main_net(combined_input)

        mean = self.mean_head(latent_pi)
        log_std = self.log_std_head(latent_pi)
        log_std = th.clamp(log_std, min=LOG_SIG_MIN, max=LOG_SIG_MAX)
...
class CAPQL(MOAgent, MOPolicy):
    ...
    def __init__(
        ...
        self.equiv_optim = optim.Adam(list(self.policy.parameters()), lr=self.learning_rate)
    ...
    def update(self):
        ...
        for _ in range(self.gradient_updates):
            ...
            policy_loss2 = ((self.alpha * log_pi) - min_q).mean()

            equivariance_loss = self.policy.compute_equivariance_loss(s_obs, w)
            policy_loss = policy_loss2 + self.lambda_loss * equivariance_loss 
    ...
    def train(
        ...
        if self.global_step == self.all_timesteps:
            self.close_wandb()
