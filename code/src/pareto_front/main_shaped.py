...
# This file is identical to main_eq except the use of CAPQL without equivariance 
def run_single_iterative_run(cfg: DictConfig, seed: int, run_id: int):
    ...
    try:
        ...
        agent = CAPQL(env=train_env, seed = seed, project_name=config['log_dir'], all_timesteps = int(config['irl']['num_refinement_cycles'] * config['irl']['refinement_timesteps']))#, learning_starts = config['rl_agent']['learning_starts'])
