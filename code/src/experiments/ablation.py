...
def _collect_episode(
    ...
    """Collect one complete episode from the sparse training environment."""
    ...
    while not (terminated or truncated):
        ...
        cumulative_sparse_reward += float(reward_vec[sparse_channel_idx])
        obs = next_obs
...
def _add_episode_to_shapers(episode: dict, shapers: list[IRLRewardShaper]) -> None:
    """Give the exact same raw trajectory and return target to every shaper."""

    for shaper in shapers:
        shaper.add_episode_data(
            episode_obs=episode["observations"],
            episode_action=episode["actions"],
            episode_true_dense_rewards=episode["true_dense_rewards"],
            cumulative_sparse_reward=episode["cumulative_sparse_reward"],
        )
...
def _make_shaper_config(
    ...
    config["irl"]["removed_indices"] = list(
...
def _collect_held_out_episodes(
    ...
    """Collect fresh final-policy trajectories not used by either reward model."""
...
def run_single_nn(cfg: DictConfig, seed: int) -> Path:
    ...
    if float(sparsity_levels[sparse_indices[0]]) != 1.0:
        raise ValueError(
            "Use sparsity 1.0 for the Walker2d forward objective so each target "
            "is the complete trajectory return."
        )
    ...
    for _ in range(initial_episodes):
        ...
        _add_episode_to_shapers(episode, shapers)
    ...
    for episode_idx in range(held_out_count):
            ...
            held_out_episodes.append(episode)

    payload = held_out_episodes#_evaluation_payload(held_out_episodes)
